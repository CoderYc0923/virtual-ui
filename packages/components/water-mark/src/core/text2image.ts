...
export const text2image: (config: FinalWaterMakeConfig) => void = (config) => {
  //判断是创建明水印还是暗水印
  config.secret ? drawEncryptedText(config) : drawSurfaceText(config);
};

//暗
const drawEncryptedText: (config: FinalWaterMakeConfig) => void = (config) => {
  ...
  const watermarkImageData = _mergeImageDate(targetImageData, textImageData);
...
//明
const drawSurfaceText: (config: FinalWaterMakeConfig) => void = (config) => {
  const [canvas] = createCanvas(config.target as HTMLImageElement);
...
const _getTextImageData: (
  config: FinalWaterMakeConfig,
  width: number,
  height: number
) => ImageData = (config, width, height) => {
  ...
  if (!ctx) throw new Error("Not exist: canvas");
...
const _mergeImageDate: (
  targetData: ImageData,
  textData: ImageData
) => ImageData = (targetData, textData) => {
