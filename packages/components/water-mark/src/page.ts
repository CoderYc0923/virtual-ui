import GuardDom from "./core/guard-dom";
import { PageConfig } from "./watermark.types";
import { getTextSize, ErrorMsg } from "./watermark.utils";
...
class PageWaterMark {
  ...
  createTextWatermark() {
    ...
    const { width } = getTextSize(config.text, config.fontSize);
    const ctx = canvas.getContext("2d");

    const dpr = window.devicePixelRatio || 1;
    canvas.width = (width + config.rowGap) * dpr;
    canvas.height = (width + config.colGap) * dpr;
  ...
  _addWatermark2Container(imgUrl: string, width?: number, height?: number) {
    this.watermakr.className = "vir-watermark";
