import { WaterMakeConfig, FinalWaterMakeConfig } from '../watermark.types'
...
export const initConfig: (config: WaterMakeConfig) => Promise<FinalWaterMakeConfig> = async (
    config
  ) => {
    //如果是给图片添加水印而不是给页面元素添加
    ...
    const configs: FinalWaterMakeConfig = {
