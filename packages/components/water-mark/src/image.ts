import { ImageConfig } from "./watermark.types";
import { getTextSize, ErrorMsg, url2img } from "./watermark.utils";
...
class ImageWaterMark {
  ...
  image2canvas: () => void = async () => {
    const { image, position, target, success, imageWidth, imageHeight } = this
      .config as ImageConfig.Image;
    const defaultImgWidth = 30;
    const defaultImgHeight = 30;
    const img = await url2img(
      image,
      imageWidth ? imageWidth : defaultImgWidth,
      imageHeight ? imageHeight : defaultImgHeight
    );
  ...
  drawEncryptedText2canvas() {
    ...
    if (ctx) {
      ...
      const textImageData = this._text2ImageData(
        this.config as ImageConfig.Text,
        width,
        height
      );
      const watermarkImageData = this._encryptAndMergeImageData(
        targetImageData,
        textImageData
      );
  ...
  _fillText2Ctx(
    ...
    let { position, color, fontSize, rowGap, colGap, angle, text } = textConfig;
    ...
    switch (position) {
      ...
      case "topLeft":
        ...
        ctx.fillText(text, 0 + rowGap, 0 + colGap);
        break;
      case "topRight":
        ctx.textAlign = "right";
        ctx.textBaseline = "top";
        ctx.fillText(text, width - rowGap, 0 + colGap);
        break;
      case "bottomRight":
        ctx.textAlign = "right";
        ctx.textBaseline = "bottom";
        ctx.fillText(text, width - rowGap, height - colGap);
        break;
      case "bottomLeft":
        ctx.textAlign = "left";
        ctx.textBaseline = "bottom";
        ctx.fillText(text, 0 + rowGap, height - colGap);
        break;
      default:
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        const { width: textWith } = getTextSize(text, fontSize);
        const wmWidth = textWith + rowGap;
        const wmHeight = textWith + colGap;
